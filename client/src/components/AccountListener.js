...
const AccountListener = ({item,i,username}) => {
  ...
  try{
    ...
    const deleteHandler = async () => {
      try {
        if (window.confirm("Bu gönderi silinmek üzere, devam etmek istiyor musunuz?")) {
  ...
  return (
    <div ref={postRef}>
        ...
        >
          {kulUsername.username!=null &&  kulUsername.username==username  &&<div style={{position:"absolute",right:"0",color:"#3cfdf0",display:"flex",paddingRight:"5px"}} ><div className="deleteButton" onClick={updateHandler} style={{paddingRight:"5px"}}>Düzenle</div>
          <div   className="deleteButton" onClick={deleteHandler}>Sil</div>
          </div>}
      ...
      {kulUsername &&  <form
        ref={formRef}
        style={{ width: "60vw", margin:"auto",display:"none",marginBottom:"50px" }}
      ...
      >
        <Form.Control
          ...
          placeholder="Başlık"
          style={{ margin: "10px 0px 2px 0px",borderRadius:"5px",fontSize:"15px" }}
        />
