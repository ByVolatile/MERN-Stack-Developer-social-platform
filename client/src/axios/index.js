...
export const GetQuestion = async(formData) => await HTTP.post('/Question/getQuestion', {
  ...
  export const toggleStar = async(formData,token) => await HTTP.post('/Question/toggleStar',{formData},{
    headers: { token }
})

export const getStar = async(formData) => await HTTP.post('/Question/getStar',{formData})

export const deletePost =async(item,kulId)=> await HTTP.delete("/Question/" + item, {
    data: {
      userId: kulId,
    },
  });
  export const updateQuestion =async(item,token)=> await HTTP.put("/Question/" + item.id, {
    data: {
      item,
      userId: token,
    },
  ...
  export const getStaredPost = async(data,token) => await HTTP.get("Question/myStars/"+data)

  export const updateUsername =async(username,newUsername,token)=> await HTTP.put("/users/updateUsername/" + username, {
    headers: { token },
    data: {
  ...
  export const follow = async(user,userId) =>  await HTTP.put("/users/follow/" + user , {
    userId
  });
  export const unfollow = async(user,userId) =>  await HTTP.put("/users/unfollow/" + user , {
    userId
  });
